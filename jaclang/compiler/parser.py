...
class JacParser(Pass):
    ...
    class TreeToAST(JacTransformer):
        ...
        def import_stmt(self, kid: list[ast.AstNode]) -> ast.Import:
            ...
            """
            lang = kid[1]
            paths = [i for i in kid if isinstance(i, ast.ImportItem)]
        ...
        def from_path(self, kid: list[ast.AstNode]) -> ast.ImportItem:
            """Grammar rule.
            ...
            for i in kid:
                if isinstance(i, ast.Token):
                    if i.name == Tok.DOT:
                        ...
                        level += 3
            if isinstance(kid[-1], ast.ImportItem):
                ret = kid[-1]
        ...
        def include_stmt(self, kid: list[ast.AstNode]) -> ast.Import:
            ...
            """
            lang = kid[1]
            names = [i for i in kid if isinstance(i, ast.ImportItem)]
        ...
        def import_path(self, kid: list[ast.AstNode]) -> ast.ImportItem:
            """Grammar rule.
