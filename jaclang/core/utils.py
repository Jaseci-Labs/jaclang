...
def get_sem_scope(node: ast.AstNode) -> SemScope:
    ...
    a = (
        ...
        else node.name.sym_name if isinstance(node, (ast.Enum, ast.Architype)) else ""
    )
    if isinstance(node, ast.Module):
                    ...
                    if isinstance(value, ast.AtomUnit):
                        ...
                        for i in value.values.items:
                            var_name = (
                                ...
                                else (i.sym_name if isinstance(i, ast.Name) else "")
                            )
